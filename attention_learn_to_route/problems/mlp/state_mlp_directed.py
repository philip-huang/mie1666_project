...
class StateMLP_Directed(NamedTuple):
    # Fixed input
    coords: torch.Tensor
    ...
    def initialize(input):
        ...
        depot = input['depot']
        loc = input['loc']
        service_times = input['service_time']

        batch_size, n_loc, _ = loc.size()
        return StateMLP_Directed(
    ...
    def update(self, selected):
        ...
        latency = self.latency + (cur_coord - self.cur_coord).norm(p=2, dim=-1) + self.service_times.gather(1, self.prev_a)  # (batch_dim, 1)
